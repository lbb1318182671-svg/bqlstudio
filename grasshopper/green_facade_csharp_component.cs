...
Set these inputs:
W                 double  default 4.2      // bay width
FloorH            double  default 3.6      // floor-to-floor height
Levels            int     default 3
CorridorD         double  default 1.45     // outdoor corridor depth
MeshGap           double  default 0.45     // distance from corridor edge to outer screen
PlantersPerLevel  int     default 3
PlanterScale      double  default 1.0
Organic           double  default 0.45     // 0.0 clean, 1.0 very blobby
Seed              int     default 7
...
Replace the generated RunScript method with the RunScript below,
then paste the helper methods into the "additional code" area.
*/
  ...
  ref object Brackets)
{
  ...
  double innerY = 0.0;
  double corridorY = -CorridorD * 0.5;
  double outerY = -CorridorD - MeshGap;

  // Inner high-performance glazing and operable door zones.
  ...
  // Outdoor corridor slabs, guardrails, and ceiling/floor edges.
  for (int i = 0; i <= Levels; i++)
  {
    double z = i * FloorH;
  ...
  // Outer expanded-metal screen as a diamond lattice.
  double dx = W / 10.0;
  ...
  // Organic precast planter modules and planting.
  ...
  for (int level = 0; level < Levels; level++)
  {
    for (int j = 0; j < PlantersPerLevel; j++)
    {
      ...
      // Back mounting plate and two structural arms.
      ...
      // Plant arcs and vine leaders clipped near the screen.
      for (int k = 0; k < 5; k++)
      {
